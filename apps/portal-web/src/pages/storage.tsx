...
export function StoragePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['storage-buckets', page, pageSize],
    queryFn: async () => {
      const r = await fetch(`/api/v1/storage/buckets?page=${page + 1}&limit=${pageSize}`, { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('accessToken') } });
