...
export function RolesPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['roles'], queryFn: async () => { const r = await fetch('/api/v1/roles', { headers: { 'Content-Type': 'application/json' } }); const j = await r.json(); const d = j?.data || j; return (d?.data || d || []); } });
  ...
  return (<div className="space-y-4"><h1 className="text-2xl font-bold">Roles</h1>
