...
import { Trash, Building2, Pencil, Filter, X } from 'lucide-react';
...
export function TenantsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['tenants', page, pageSize, debouncedSearch, sorting, activeFilters],
    queryFn: async () => {
      ...
      const r = await fetch(`/api/v1/tenants?${params.toString()}`, { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('accessToken') } });
  ...
  const filterFields: FilterField[] = useMemo(() => [
    ...
    { id: 'plan', label: 'Plan', type: 'select', options: [{ label: 'Free', value: 'FREE' }, { label: 'Pro', value: 'PRO' }, { label: 'Enterprise', value: 'ENTERPRISE' }] },
  ], []);
  ...
  return (<div className="h-full flex flex-col space-y-4 overflow-hidden">
    <div className="flex items-center justify-between"><h1 className="text-2xl font-bold">Tenants</h1>
      <div className="flex items-center gap-2">
        ...
        <Button onClick={() => { setDialogOpen(true); }}><Building2 size={16} className="mr-1" /> Add Tenant</Button>
      </div></div>
