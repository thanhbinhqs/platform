...
export function ScheduledJobsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['scheduled-jobs'], queryFn: async () => { const r = await fetch('/api/v1/scheduled-jobs', { headers: { 'Content-Type': 'application/json' } }); const j = await r.json(); const d = j?.data || j; return (d?.data || d || []); } });
  ...
  return (<div className="space-y-4"><h1 className="text-2xl font-bold">Scheduled Jobs</h1>
