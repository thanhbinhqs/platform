...
export function DelegationsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['delegations'], queryFn: async () => { const r = await fetch('/api/v1/delegations', { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('accessToken') } }); const j = await r.json(); const d = j?.data || j; return (d?.data || d || []) as Item[]; } });
