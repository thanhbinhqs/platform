...
export function WorkflowsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await fetch(`/api/v1/workflows?page=${page + 1}&limit=${pageSize}`, { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('accessToken') } });
