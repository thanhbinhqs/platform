...
export function WorkflowsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['workflows'], queryFn: async () => { const r = await fetch('/api/v1/workflows', { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('accessToken') } }); const j = await r.json(); const d = j?.data || j; return (d?.data || d || []); } });
  ...
  return (<div className="h-full flex flex-col space-y-4 overflow-hidden"><h1 className="text-2xl font-bold">Workflows</h1>
