...
import { FileText, Upload, RefreshCw, Eye, Pencil, XCircle, Trash } from 'lucide-react';
...
export function OrdersPage() {
  ...
  const columns = useMemo<any>(() => [
    ...
    { accessorKey: 'total', header: 'Total', cell: (info: any) => `$${Number(info.getValue()).toFixed(2)}` },
  ...
  const filterFields = useMemo(() => [
    { id: 'orderNumber', label: 'Order #', type: 'text' as const },
    { id: 'customerName', label: 'Customer', type: 'text' as const },
    { id: 'total', label: 'Total', type: 'number-range' as const },
    { id: 'status', label: 'Status', type: 'select' as const, options: [{ label: 'Pending', value: 'PENDING' }, { label: 'Confirmed', value: 'CONFIRMED' }, { label: 'Shipped', value: 'SHIPPED' }, { label: 'Delivered', value: 'DELIVERED' }, { label: 'Cancelled', value: 'CANCELLED' }] },
  ], []);
  ...
  return (<div className="h-full flex flex-col">
    <AppDataGrid
      columns={columns} data={data?.items || []} title="Orders"
      ...
      bulkActions={[
        { label: 'Cancel', icon: <Trash size={14} />, onClick: (ids: string[]) => { if (confirm(`Cancel ${ids.length} orders?`)) bulkDeleteMutation.mutate(ids); } },
      ]}
      tableActions={[
        { label: 'Add Order', icon: <FileText size={14} />, onClick: () => { setEditItem(null); setDialogOpen(true); }, variant: 'primary' as const },
