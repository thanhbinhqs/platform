...
export function OrdersPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['sales-orders'], queryFn: async () => { const r = await fetch('/api/v1/sales/orders', { headers: { 'Content-Type': 'application/json' } }); const j = await r.json(); const d = j?.data || j; return (d?.data || d || []); } });
