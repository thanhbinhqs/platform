...
export function WebhooksPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['webhooks'], queryFn: async () => { const r = await fetch('/api/v1/webhooks', { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('accessToken') } }); const j = await r.json(); const d = j?.data || j; return (d?.data || d || []) as Item[]; } });
  ...
  return (<div className="h-full flex flex-col space-y-4 overflow-hidden">
    ...
    <DataGrid columns={columns} data={data || []} title="Webhooks" enableSorting enableColumnVisibility enableExport enableDensity enableRowNumber onSelectionChange={setSelection} pageSize={15} pageSizeOptions={[10, 15, 25, 50, 100]} emptyMessage="No webhooks found." />
