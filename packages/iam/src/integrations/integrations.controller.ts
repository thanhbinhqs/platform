...
export class IntegrationsController {
  ...
  async create(@Body() b: any): Promise<any> {
    return this.prisma.client.integration.create({ data: { name: b.name, type: b.type, provider: b.provider || 'MANUAL', config: b.config || {}, status: 'DISCONNECTED' } as any });
  }
  ...
  async update(@Param('id') id: string, @Body() b: any): Promise<any> {
    return this.prisma.client.integration.update({ where: { id }, data: b });
  }
  ...
  async remove(@Param('id') id: string): Promise<any> {
    return this.prisma.client.integration.update({ where: { id }, data: { status: 'DISCONNECTED' as any } });
  }
  ...
  async bulkRemove(@Body() dto: BulkIdsDto): Promise<any> {
    return this.prisma.client.integration.updateMany({
      where: { id: { in: dto.ids } },
